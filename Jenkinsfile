artifacts/x/xa/xy/batch.sql
SELECT 'dummy-artifact-build' AS status;
...
#     <env>-artifacts.json (optional, preferred)
...
ARTIFACTS_FILE_PATH="$(resolve_env_scoped_json "$WORKFLOW_DIR" "$ENV_NAME" "artifacts")"
...
RENDERED_ARTIFACTS="${TMP_DIR}/artifacts.rendered.json"
...
if [[ -n "$ARTIFACTS_FILE_PATH" ]]; then
  python3 scripts/render_asl.py "$ENV_FILE" "$ARTIFACTS_FILE_PATH" "$RENDERED_ARTIFACTS"
fi
...
resolve_s3_destination_bucket_and_key() {
  ...
  if [[ -z "$key" ]]; then
    ...
    key="$(join_s3_key "$key" "$default_file_name")"
  fi

  printf '%s\n%s\n' "$bucket" "$key"
}

backup_existing_s3_object_if_needed() {
  local bucket="$1"
  local key="$2"
  ...
  if aws s3api head-object --region "$AWS_REGION" --bucket "$bucket" --key "$key" >/dev/null 2>&1; then
    local backup_suffix backup_key
    backup_suffix="$(date -u +"$backup_format")"
    backup_key="${key}.${backup_suffix}"
    echo "==> Backing up existing object: s3://${bucket}/${key} -> s3://${bucket}/${backup_key}"
...
PREV_ARTIFACTS_HASH="$(jq -r --arg id "$WF_ID" '.workflows[$id].artifacts.hash // empty' "$STATE_FILE_LOCAL")"
      ...
      case "$COPY_MODE" in
        files)
          ...
          for rel_path in "${FILES_TO_COPY[@]}"; do
            ...
            if [[ "$BACKUP_EXISTING" == "true" ]]; then
              backup_existing_s3_object_if_needed "$DEST_BUCKET" "$dst_key" "$BACKUP_EXISTING" "$BACKUP_FORMAT"
            fi
...
ARTIFACTS_HASH=""
ARTIFACTS_COMMENT=""
if [[ -f "$RENDERED_ARTIFACTS" ]]; then
  ARTIFACTS_HASH="$(sha256_of_file "$RENDERED_ARTIFACTS")"
  ARTIFACTS_COMMENT="$(jq -r '.comment // empty' "$RENDERED_ARTIFACTS")"
  RUN_ARTIFACTS="true"
  if [[ -n "$PREV_ARTIFACTS_HASH" && "$PREV_ARTIFACTS_HASH" == "$ARTIFACTS_HASH" ]]; then
    RUN_ARTIFACTS="false"
  fi

  if [[ "$RUN_ARTIFACTS" == "false" ]]; then
    echo "==> Artifacts unchanged by state hash; skipping artifacts tasks."
  else
    mapfile -t ARTIFACT_TASKS < <(jq -c 'if has("tasks") then .tasks[]? else . end' "$RENDERED_ARTIFACTS")
    if (( ${#ARTIFACT_TASKS[@]} > 0 )); then
      for task_index in "${!ARTIFACT_TASKS[@]}"; do
        task_json="${ARTIFACT_TASKS[$task_index]}"
        TASK_ENABLED="$(jq -r '.enabled // true' <<< "$task_json")"
        if [[ "$TASK_ENABLED" != "true" ]]; then
          echo "==> Skipping artifacts task #$((task_index + 1)) (enabled=false)"
          continue
        fi

        TASK_MODE="$(jq -r '.mode // empty' <<< "$task_json")"
        SOURCE_SQL_PATH="$(jq -r '.sourceSqlPath // empty' <<< "$task_json")"
        SOURCE_FILE_PATH="$(jq -r '.sourceFilePath // empty' <<< "$task_json")"
        DESTINATION_URI="$(jq -r '.destinationUri // empty' <<< "$task_json")"
        BACKUP_EXISTING="$(jq -r '.backupExisting // true' <<< "$task_json")"
        BACKUP_FORMAT="$(jq -r '.backupTimestampFormat // "%Y%m%d%H%M%S"' <<< "$task_json")"

        if [[ -z "$TASK_MODE" || "$TASK_MODE" == "null" ]]; then
        ...
        if [[ -z "$DESTINATION_URI" || "$DESTINATION_URI" != s3://* ]]; then
          echo "artifacts task #$((task_index + 1)) requires destinationUri starting with s3://" >&2
          exit 2
        fi

        case "$TASK_MODE" in
          tar)
            if [[ -z "$SOURCE_SQL_PATH" ]]; then
              echo "artifacts task #$((task_index + 1)) mode=tar requires sourceSqlPath" >&2
              exit 2
            fi
            if [[ "$SOURCE_SQL_PATH" == s3://* ]]; then
              echo "artifacts.sourceSqlPath must be a local repository path: $SOURCE_SQL_PATH" >&2
              exit 2
            fi
            if [[ ! -f "$SOURCE_SQL_PATH" ]]; then
              echo "artifacts.sourceSqlPath does not exist: $SOURCE_SQL_PATH" >&2
              exit 2
            fi

            source_dir="$(dirname "$SOURCE_SQL_PATH")"
            source_file="$(basename "$SOURCE_SQL_PATH")"
            artifact_name="$(basename "$source_dir").tar.gz"
            artifact_local_path="${TMP_DIR}/artifact-${task_index}-${artifact_name}"

            echo "==> Building artifact for task #$((task_index + 1)): $artifact_name from $SOURCE_SQL_PATH"
            (
              cd "$source_dir"
              tar -czf "$artifact_local_path" "$source_file"
            )

            mapfile -t DEST_PARTS < <(resolve_s3_destination_bucket_and_key "$DESTINATION_URI" "$artifact_name")
            DEST_BUCKET="${DEST_PARTS[0]}"
            DEST_KEY="${DEST_PARTS[1]}"
            if [[ -z "${DEST_BUCKET:-}" || -z "${DEST_KEY:-}" ]]; then
              echo "Failed to resolve artifacts destinationUri for task #$((task_index + 1)): $DESTINATION_URI" >&2
              exit 2
            fi
            backup_existing_s3_object_if_needed "$DEST_BUCKET" "$DEST_KEY" "$BACKUP_EXISTING" "$BACKUP_FORMAT"

            echo "==> Uploading artifact: ${artifact_local_path} -> s3://${DEST_BUCKET}/${DEST_KEY}"
            aws s3 cp "$artifact_local_path" "s3://${DEST_BUCKET}/${DEST_KEY}"
            ;;
          copy)
            if [[ -z "$SOURCE_FILE_PATH" ]]; then
              echo "artifacts task #$((task_index + 1)) mode=copy requires sourceFilePath" >&2
              exit 2
            fi
            if [[ "$SOURCE_FILE_PATH" == s3://* ]]; then
              echo "artifacts.sourceFilePath must be a local repository path: $SOURCE_FILE_PATH" >&2
              exit 2
            fi
            if [[ ! -f "$SOURCE_FILE_PATH" ]]; then
              echo "artifacts.sourceFilePath does not exist: $SOURCE_FILE_PATH" >&2
              exit 2
            fi

            source_file_name="$(basename "$SOURCE_FILE_PATH")"
            mapfile -t DEST_PARTS < <(resolve_s3_destination_bucket_and_key "$DESTINATION_URI" "$source_file_name")
            DEST_BUCKET="${DEST_PARTS[0]}"
            DEST_KEY="${DEST_PARTS[1]}"
            if [[ -z "${DEST_BUCKET:-}" || -z "${DEST_KEY:-}" ]]; then
              echo "Failed to resolve artifacts destinationUri for task #$((task_index + 1)): $DESTINATION_URI" >&2
            ...
            echo "==> Copying artifact source file: ${SOURCE_FILE_PATH} -> s3://${DEST_BUCKET}/${DEST_KEY}"
            aws s3 cp "$SOURCE_FILE_PATH" "s3://${DEST_BUCKET}/${DEST_KEY}"
            ;;
          *)
            echo "Unsupported artifacts.mode: $TASK_MODE (supported: tar, copy)" >&2
            exit 2
            ;;
        esac
      done
...
if [[ -n "$ARTIFACTS_HASH" ]]; then
  ARTIFACT_TASK_COUNT="$(jq -r 'if has("tasks") then (.tasks | length) else 1 end' "$RENDERED_ARTIFACTS")"
  STATE_UPDATED_NEXT="${TMP_DIR}/deploy-state.updated.artifacts.json"
  jq -c \
    --arg id "$WF_ID" \
    --arg hash "$ARTIFACTS_HASH" \
    --arg comment "$ARTIFACTS_COMMENT" \
    --arg updated_at "$UPDATED_AT" \
    --argjson task_count "$ARTIFACT_TASK_COUNT" \
    '
  ...
  STATE_UPDATED_NEXT="${TMP_DIR}/deploy-state.updated.artifacts-removed.json"
  jq -c --arg id "$WF_ID" '
    if (.workflows[$id] // null) == null then
      .
    else
      .workflows[$id] |= del(.artifacts)
    end
  ' "$STATE_UPDATED" > "$STATE_UPDATED_NEXT"
  mv "$STATE_UPDATED_NEXT" "$STATE_UPDATED"
fi
