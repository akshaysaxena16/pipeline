...
- Workflow config: `stepfunctions/workflows/initial-env-test/cert-artifacts.json`
  and `stepfunctions/workflows/initial-env-test/prod-artifacts.json`
- Task behavior:
...
**Recommended (env placeholder in folder name):** put the literal segment `env` between hyphens in the workflow folder name, for example **`initial-env-test`**. At deploy time, `-env-` is replaced with `-cert-` or `-prod-`:

- State machine: `initial-cert-test` (cert) / `initial-prod-test` (prod)
- EventBridge rule: `initial-cert-test-trigger` / `initial-prod-test-trigger`

**Legacy (no `-env-` in folder name):** if the workflow id does **not** contain `-env-`, defaults are unchanged:

- State machine: `sfn-__ENV__-__WORKFLOW__` (for example `sfn-cert-my-workflow`)
- EventBridge rule: `__ENV__-__WORKFLOW__-trigger` (for example `cert-my-workflow-trigger`)

**Overrides (optional):**

- `SFN_STATE_MACHINE_NAME_TEMPLATE` — if set, substitutes `__ENV__`, `__WORKFLOW__`, `${ENV}`, `${WORKFLOW}` (same as before).
- `EVENTBRIDGE_RULE_NAME_TEMPLATE` — if set, same substitution. If unset and the workflow id contains `-env-`, the rule name is `<stateMachineName>-trigger`.
- Event bus: `EVENTBRIDGE_EVENT_BUS_NAME` (default: `default`)

`__WORKFLOW__` is the workflow folder name; `__ENV__` is the deployment environment (`cert` or `prod`).

### Migration note (rename / old CI templates)

If you previously deployed with `SFN_STATE_MACHINE_NAME_TEMPLATE: dummy-__ENV__-__WORKFLOW__` (for example state machine `dummy-cert-initial-env-test`), a git rename that emits `DELETE:initial-env-test` runs `delete_sfn.sh` **without** that template and will look for the **legacy** name `sfn-cert-initial-env-test` instead. Clean up the old AWS resources manually, or run delete once with the templates that match how they were created, for example:

`SFN_STATE_MACHINE_NAME_TEMPLATE='dummy-__ENV__-__WORKFLOW__' EVENTBRIDGE_RULE_NAME_TEMPLATE='dummy-__ENV__-__WORKFLOW__-trigger' scripts/delete_sfn.sh cert initial-env-test`
